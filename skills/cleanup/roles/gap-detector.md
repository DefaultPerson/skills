...
Then classify, quoting the exact text from both files — if you cannot quote the rewritten equivalent, it IS missing, and a finding without quotes is not a finding:

- Same meaning, anywhere in the file → skip.
- Present but a concrete detail is gone → **PARTIAL**. `Pricing: $50/mo, 14-day trial` → `Pricing: $50/mo` loses the trial.
- Meaning changed or inverted → **REVERSED**.
- Nowhere in the file → **MISSING**.

Surface edits are never gaps: grammar, capitalisation, punctuation, typo fixes, bullet↔checkbox, link text, merged adjacent sentences, or any rephrasing that keeps the idea and its details.
